...
fn dequant_expert(gguf: &GgufFile, name: &str, expert: usize,
    ...
    let (bs, bpb) = match info.ggml_type {
        GgmlType::Q6_K => (reinstinct_quant::q6_k::BLOCK_SIZE, reinstinct_quant::q6_k::BYTES_PER_BLOCK),
        GgmlType::Q8_0 => (reinstinct_quant::q8_0::BLOCK_SIZE, reinstinct_quant::q8_0::BYTES_PER_BLOCK),
        GgmlType::Q5_K => (reinstinct_quant::q5_k::BLOCK_SIZE, reinstinct_quant::q5_k::BYTES_PER_BLOCK),
        GgmlType::Q4_K => (reinstinct_quant::q4_k::BLOCK_SIZE, reinstinct_quant::q4_k::BYTES_PER_BLOCK),
    ...
    match info.ggml_type {
        GgmlType::Q6_K => reinstinct_quant::q6_k::dequantize_to_f32(slice, &mut out),
        GgmlType::Q8_0 => reinstinct_quant::q8_0::dequantize_to_f32(slice, &mut out),
        GgmlType::Q5_K => reinstinct_quant::q5_k::dequantize_to_f32(slice, &mut out),
        GgmlType::Q4_K => reinstinct_quant::q4_k::dequantize_to_f32(slice, &mut out),
        _ => unreachable!(),
    }
    Ok(out)
}
...
fn dequant(gguf: &GgufFile, name: &str) -> Result<Vec<f32>, String> {
    ...
    let bytes = gguf.tensor_data(name)
        .map_err(|e| format!("read {name}: {e}"))?
        .ok_or_else(|| format!("no data for {name}"))?;
    reinstinct_quant::dequantize_tensor(info, bytes).map_err(|e| format!("dequant {name}: {e}"))
}
