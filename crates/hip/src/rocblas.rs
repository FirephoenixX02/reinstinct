...
mod tests {
    use super::*;
    use crate::{DeviceBuf, Stream};

    /// Minimal f32→f16 conversion (round-to-nearest-even).
    fn f32_to_f16(v: f32) -> u16 {
        let bits = v.to_bits();
        let sign = (bits >> 16) as u16 & 0x8000;
        let exp = ((bits >> 23) as i32) - 127 + 15;
    ...
    fn f16_to_f32(v: u16) -> f32 {
        ...
        let mant = ((v & 0x3FF) as u32) << 13;
        if exp <= 0 { return f32::from_bits(sign); }
        if exp >= 255 { return f32::from_bits(sign | 0x7F80_0000); }
    ...
    fn hgemm_matches_cpu_reference() {
        // Skipped: the hip crate doesn't depend on quant::half, so we
        // can't do f16 conversion here. This test is validated via the
        // runtime crate's integration tests which have both hip and quant.
        eprintln!("skip: hgemm test needs quant::half (circular dep)");
        return;
        if crate::device_count().ok().unwrap_or(0) < 1 { eprintln!("skip"); return; }
        let _dev = crate::Device::set(0).unwrap();
