...
use reinstinct_gguf::{GgmlType, GgufFile};
use reinstinct_hip::{DeviceBuf, Event, Graph, GraphExec, Module, Stream};
use reinstinct_hip::sys::HipStreamCaptureMode;
use reinstinct_model::gemma4::{AttnKind, Gemma4Model};
use crate::KernelCache;
use crate::qwen35::{DeviceBufPool, GpuMatvecTensor, PooledBuf};
...
fn load_bf16_as_f32_matvec(gguf: &GgufFile, name: &str) -> Result<GpuMatvecTensor, String> {
    ...
    let f32s: Vec<f32> = src.iter().map(|&b| reinstinct_quant::half::bf16_to_f32(b)).collect();
    Ok(GpuMatvecTensor {
...
impl ExpertTensor {
    fn from_gguf(gguf: &GgufFile, name: &str) -> Result<Self, String> {
        ...
        if info.ggml_type == GgmlType::Q6_K {
            ...
            for e in 0..n_expert {
                packed.extend_from_slice(&reinstinct_quant::q6_k::repack_for_matvec(
    ...
    fn from_gguf_repacked(gguf: &GgufFile, name: &str) -> Result<Self, String> {
        ...
        for e in 0..n_expert {
            ...
            let rep = match info.ggml_type {
                GgmlType::Q6_K => reinstinct_quant::q6_k::repack_for_matvec(slice, in_dim, out_dim),
                GgmlType::Q8_0 => reinstinct_quant::q8_0::repack_for_matvec(slice, in_dim, out_dim),
...
pub struct Gemma4GpuState {
    ...
    pub superquant: Option<Vec<crate::kv_superquant::SuperQuantKvCache>>,
    pub pos: usize,
...
impl Gemma4GpuState {
    ...
    pub fn new_with_superquant(
        model: &Gemma4Model,
        max_seq: usize,
        kernel_cache: &crate::KernelCache,
        config: crate::kv_superquant::SuperQuantConfig,
    ) -> Result<Self, String> {
        use crate::kv_superquant::SuperQuantKvCache;
    ...
    pub fn migrate_int8_to_superquant(&self, kernel_cache: &crate::KernelCache)
        -> Result<(), String>
    {
        ...
        for (i, (c, sq)) in self.caches.iter().zip(sq_caches.iter()).enumerate() {
            ...
            if n_cold > 0 {
                use crate::kv_turbo3::launch_promote_q8_to_turbo3;
...
pub struct GpuGemma4 {
    ...
    /// prefill latency.
    rocblas:       reinstinct_hip::rocblas::Handle,
    prefill_gemm:  crate::prefill::PrefillGemm,
    m_rope_pf:     Module,
    m_attn_pf:     Module,
    m_kvq_pf:      Module,
...
impl GpuGemma4 {
        ...
        -> Result<Self, String>
    {
        ...
        let build_rope = |rotary: usize, base: f32| -> Result<(DeviceBuf<f32>, DeviceBuf<f32>), String> {
            let rc = reinstinct_cpu::rope::RopeCache::new(rotary, max_seq, base);
        ...
        let rocblas = reinstinct_hip::rocblas::Handle::new()?;
        rocblas.set_stream(&stream)?;
        let prefill_gemm = crate::prefill::PrefillGemm::new(
    ...
    pub(crate) fn launch_attn_superquant_wp(&self,
        q: *mut c_void,
        kv: &crate::kv_superquant::SuperQuantKvCache,
        out: *mut c_void,
        n_kv: u32, head_dim: u32) -> Result<(), String>
    {
