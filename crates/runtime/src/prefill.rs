...
use reinstinct_gguf::GgmlType;
use reinstinct_hip::{self, DeviceBuf, Module};
use reinstinct_hip::rocblas::{self, Handle, RocblasOp};
    ...
    -> Result<DeviceBuf<f32>, String>
{
    ...
    let cvt = |f: &reinstinct_hip::Function, src: *mut c_void, dst: *mut c_void, n: u32|
        -> Result<(), String> {
        let block: u32 = 256;
...
impl PrefillGemm {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn matmul(&self, handle: &Handle, stream: &reinstinct_hip::Stream,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn matmul_into(&self, handle: &Handle, stream: &reinstinct_hip::Stream,
                       dst: &DeviceBuf<f32>,
                       w_dev: &DeviceBuf<u8>, dtype: GgmlType, repacked: bool,
                       in_dim: usize, out_dim: usize,
                       x: &DeviceBuf<f32>, n_rows: usize)
        -> Result<(), String>
    {
        ...
        let to_f16 = self.cvt.function("cvt_f32_to_f16")?;
        let to_f32 = self.cvt.function("cvt_f16_to_f32")?;
        let cvt = |f: &reinstinct_hip::Function, src: *mut c_void, ddst: *mut c_void, n: u32|
            -> Result<(), String> {
            let block: u32 = 256;
    ...
    fn grow(buf: &std::cell::RefCell<DeviceBuf<u16>>, n: usize, stream: &reinstinct_hip::Stream)
        -> Result<(), String>
    {
        if buf.borrow().len() < n {
    ...
    fn matmul_mmq(&self, stream: &reinstinct_hip::Stream, w_dev: &DeviceBuf<u8>, dtype: GgmlType,
                  in_dim: usize, out_dim: usize,
                  x: &DeviceBuf<f32>, n_rows: usize)
        -> Result<DeviceBuf<f32>, String>
    {
    ...
    #[allow(clippy::too_many_arguments)]
    fn matmul_kquant_batched_into(&self, stream: &reinstinct_hip::Stream, dst: &DeviceBuf<f32>,
                                  w_dev: &DeviceBuf<u8>, dtype: GgmlType,
                                  in_dim: usize, out_dim: usize,
                                  x: &DeviceBuf<f32>, n_rows: usize)
        -> Result<(), String>
    ...
    #[allow(clippy::too_many_arguments)]
    fn matmul_mmq_into(&self, stream: &reinstinct_hip::Stream, dst: &DeviceBuf<f32>,
                       w_dev: &DeviceBuf<u8>, dtype: GgmlType,
                       in_dim: usize, out_dim: usize,
                       x: &DeviceBuf<f32>, n_rows: usize)
        -> Result<(), String>
...
mod tests {
    ...
    fn batched_matmul_matches_sequential_q4_k() {
        ...
        for blk in 0..n_blocks {
            ...
            w[off..off+2].copy_from_slice(&reinstinct_quant::half::f32_to_f16(0.01).to_le_bytes());
            w[off+2..off+4].copy_from_slice(&reinstinct_quant::half::f32_to_f16(0.005).to_le_bytes());
