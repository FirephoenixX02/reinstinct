...
#[cfg(test)]
use reinstinct_cpu::qwen3_5::Qwen35F32Model;
use reinstinct_model::qwen3_5::Qwen35Model;
use reinstinct_gguf::{GgufFile, GgmlType};
#[cfg_attr(not(test), allow(unused_imports))]
use reinstinct_hip::{self, DeviceBuf, Event, Graph, GraphExec, Module, Stream};
use reinstinct_hip::sys::HipStreamCaptureMode;
use reinstinct_hip::rocblas::{Handle as RocblasHandle, RocblasOp};
...
impl GpuMatvecTensor {
    ...
    pub fn from_gguf_matvec(gguf: &GgufFile, name: &str) -> Result<Self, String> {
        ...
        let packed = match info.ggml_type {
            GgmlType::Q4_K => Some(reinstinct_quant::q4_k::repack_for_matvec(
                bytes, in_dim as usize, out_dim as usize)),
            GgmlType::Q5_K => Some(reinstinct_quant::q5_k::repack_for_matvec(
                bytes, in_dim as usize, out_dim as usize)),
            GgmlType::Q6_K => Some(reinstinct_quant::q6_k::repack_for_matvec(
                bytes, in_dim as usize, out_dim as usize)),
            GgmlType::Q8_0 => Some(reinstinct_quant::q8_0::repack_for_matvec(
                bytes, in_dim as usize, out_dim as usize)),
            _ => None,
        };
        match packed {
...
impl GpuExpertTensor {
    pub fn from_gguf(gguf: &GgufFile, name: &str) -> Result<Self, String> {
        ...
        let repack_one = |slice: &[u8]| -> Option<Vec<u8>> {
            match info.ggml_type {
                GgmlType::Q4_K => Some(reinstinct_quant::q4_k::repack_for_matvec(slice, in_dim, out_dim)),
                GgmlType::Q5_K => Some(reinstinct_quant::q5_k::repack_for_matvec(slice, in_dim, out_dim)),
                GgmlType::Q6_K => Some(reinstinct_quant::q6_k::repack_for_matvec(slice, in_dim, out_dim)),
                _ => None,
            }
        };
...
impl MoeRuntime {
    fn new(moe: &reinstinct_model::qwen3_5::MoeConfig, hidden: usize, cache: &KernelCache)
        -> Result<Self, String>
    {
...
impl GpuBlock {
    pub fn from_gguf(gguf: &GgufFile, layer: u32, kind: reinstinct_model::qwen3_5::BlockKind,
...
impl GpuQwen35 {
        ...
        -> Result<Self, String>
    {
        ...
        let rope = reinstinct_cpu::rope::RopeCache::new(rotary_dim, max_seq, cfg.rope_freq_base);
        ...
        -> Result<(f32, usize, usize), String>
    {
        ...
        for i in 0..n_tokens {
            ...
            let next = reinstinct_sampling::argmax(&main_logits);
            ...
            pending = Some(reinstinct_sampling::argmax(&logits_host));
            tok = next;
        }
    ...
    ) -> Result<(Vec<u32>, QwenSpecStats), String>
    {
        ...
        while generated.len() < max_tokens {
            let t = reinstinct_sampling::argmax(&verify_logits);
            snapshot.save(state)?;
            ...
            let all_ok = (0..k).all(|i|
                reinstinct_sampling::argmax(&verify_out[i]) == drafts[i]);
...
mod tests {
    ...
    fn embed_norm_proj_matches_cpu_chain() {
        if reinstinct_hip::device_count().ok().unwrap_or(0) < 1 { eprintln!("skip: no HIP device"); return; }
        let _dev = reinstinct_hip::Device::set(0).unwrap();
        ...
        use reinstinct_cpu::qwen3_5::dequant_named;
        ...
        for &token in &[cfg.eos_token_id, 100u32, 50_000u32] {
            ...
            reinstinct_cpu::ops::rmsnorm(embed, &output_norm, cfg.rms_norm_eps, &mut normed);
            let proj_w = output.as_deref().unwrap_or(token_embd.as_slice());
            let mut cpu_logits = vec![0.0f32; vocab];
            reinstinct_cpu::ops::matvec(&normed, proj_w, hidden, vocab, &mut cpu_logits);
    ...
    fn swiglu_ffn_matches_cpu_for_real_block() {
        ...
        use reinstinct_cpu::qwen3_5::BlockWeights;
        ...
        for &block_idx in &block_indices {
            ...
            reinstinct_cpu::qwen3_5::swiglu_ffn(&input, gate_w, up_w, down_w, h, f, &mut cpu_out);
    ...
    fn forward_tokens_batched_matches_sequential() {
        if reinstinct_hip::device_count().ok().unwrap_or(0) < 1 { eprintln!("skip: no HIP"); return; }
        let _dev = reinstinct_hip::Device::set(0).unwrap();
    ...
    fn forward_tokens_matches_repeated_forward_token_gpu() {
        ...
        if reinstinct_hip::device_count().ok().unwrap_or(0) < 1 { eprintln!("skip"); return; }
        let _dev = reinstinct_hip::Device::set(0).unwrap();
    ...
    fn linear_attention_step_matches_cpu_for_real_block() {
        ...
        use reinstinct_cpu::qwen3_5::{LinAttnState as CpuLinAttnState,
        ...
        let block_idx = model.block_kinds.iter()
            .position(|k| matches!(k, reinstinct_model::qwen3_5::BlockKind::LinearAttention))
    ...
    fn full_attention_block_matches_cpu_for_real_block() {
        ...
        use reinstinct_cpu::qwen3_5::{LayerKvCache, full_attention_block, load_full_attention};
        use reinstinct_cpu::rope::RopeCache;
        ...
        let block_idx = model.block_kinds.iter()
            .position(|k| matches!(k, reinstinct_model::qwen3_5::BlockKind::FullAttention))
    ...
    fn full_attention_step_matches_cpu_for_real_block() {
        ...
        use reinstinct_cpu::qwen3_5::{BlockWeights, LayerKvCache, full_attention_step};
        use reinstinct_cpu::rope::RopeCache;
