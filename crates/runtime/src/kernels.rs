...
use reinstinct_hip::{self, DeviceBuf, Module};
                        ...
                        window: u32) -> Result<Vec<f32>, String>
{
    ...
    reinstinct_hip::Device(0).synchronize()?;
...
pub fn matvec_iq4_xs_f32(cache: &KernelCache, w_bytes: &[u8], x: &[f32],
                         in_dim: usize, out_dim: usize) -> Result<Vec<f32>, String>
{
    use reinstinct_quant::iq4_xs::{BLOCK_SIZE, BYTES_PER_BLOCK};
...
pub fn matvec_q5_k_f32(cache: &KernelCache, w_bytes: &[u8], x: &[f32],
                       in_dim: usize, out_dim: usize) -> Result<Vec<f32>, String>
{
    use reinstinct_quant::q5_k::{BLOCK_SIZE, BYTES_PER_BLOCK};
    assert_eq!(in_dim % BLOCK_SIZE, 0, "in_dim must be a multiple of {}", BLOCK_SIZE);
    let n_blocks = in_dim / BLOCK_SIZE;
    let expect_bytes = out_dim * n_blocks * BYTES_PER_BLOCK;
    assert_eq!(w_bytes.len(), expect_bytes,
...
pub fn matvec_q6_k_f32(cache: &KernelCache, w_bytes: &[u8], x: &[f32],
                       in_dim: usize, out_dim: usize) -> Result<Vec<f32>, String>
{
    use reinstinct_quant::q6_k::{BLOCK_SIZE, BYTES_PER_BLOCK};
    assert_eq!(in_dim % BLOCK_SIZE, 0, "in_dim must be a multiple of {}", BLOCK_SIZE);
    let n_blocks = in_dim / BLOCK_SIZE;
    let expect_bytes = out_dim * n_blocks * BYTES_PER_BLOCK;
    assert_eq!(w_bytes.len(), expect_bytes,
...
pub fn matvec_q4_k_f32(cache: &KernelCache, w_bytes: &[u8], x: &[f32],
                       in_dim: usize, out_dim: usize) -> Result<Vec<f32>, String>
{
    use reinstinct_quant::q4_k::{BLOCK_SIZE, BYTES_PER_BLOCK};
    assert_eq!(in_dim % BLOCK_SIZE, 0, "in_dim must be a multiple of {}", BLOCK_SIZE);
    let n_blocks = in_dim / BLOCK_SIZE;
    let expect_bytes = out_dim * n_blocks * BYTES_PER_BLOCK;
    assert_eq!(w_bytes.len(), expect_bytes,
...
pub fn matvec_q8_0_f32(cache: &KernelCache, w_bytes: &[u8], x: &[f32],
                       in_dim: usize, out_dim: usize) -> Result<Vec<f32>, String>
{
    use reinstinct_quant::q8_0::{BLOCK_SIZE, BYTES_PER_BLOCK};
    assert_eq!(in_dim % BLOCK_SIZE, 0, "in_dim must be a multiple of {}", BLOCK_SIZE);
    let n_blocks = in_dim / BLOCK_SIZE;
    let expect_bytes = out_dim * n_blocks * BYTES_PER_BLOCK;
    assert_eq!(w_bytes.len(), expect_bytes,
...
pub fn turbo3_encode(cache: &KernelCache,
                     x: &[f32],
                     head_dim: usize,
                     kind: reinstinct_quant::turbo3::CacheKind)
...
mod tests {
    use super::*;
    use reinstinct_cpu::ops::rmsnorm as cpu_rmsnorm;

    fn skip_if_no_gpu() -> Option<KernelCache> {
        if reinstinct_hip::device_count().ok().unwrap_or(0) < 1 {
            eprintln!("skip: no HIP device"); return None;
        }
        let _ = reinstinct_hip::Device::set(0).ok()?;
    ...
    fn turbo3_encode_matches_cpu_bit_identical() {
        let Some(cache) = skip_if_no_gpu() else { return };
        use reinstinct_quant::turbo3::{CacheKind, encode_rows};
    ...
    fn turbo3_decode_matches_cpu_and_snr_floor() {
        let Some(cache) = skip_if_no_gpu() else { return };
        use reinstinct_quant::turbo3::{CacheKind, encode_rows, decode_rows};
    ...
    fn attn_step_matches_cpu_oracle() {
        ...
        for &total_len in &[1usize, 4, 17, 64] {
            ...
            for h in 0..n_heads {
                ...
                reinstinct_cpu::ops::softmax(&mut scores);
    ...
    fn rope_batched_matches_per_position() {
        let Some(cache) = skip_if_no_gpu() else { return };
        use reinstinct_cpu::rope::{RopeCache, apply_rope};
        let head_dim = 256usize;
    ...
    fn attn_step_batched_matches_per_row() {
        ...
        for r in 0..n_rows {
            ...
            for h in 0..n_heads {
                ...
                reinstinct_cpu::ops::softmax(&mut scores[..tl]);
    ...
    fn geglu_matches_cpu() {
        ...
        for i in 0..n {
            let want = reinstinct_cpu::ops::gelu(gate[i]) * up[i];
    ...
    fn attn_step_window_matches_cpu() {
        ...
        for h in 0..n_heads {
            ...
            reinstinct_cpu::ops::softmax(&mut sc);
    ...
    fn swiglu_matches_cpu() {
        ...
        reinstinct_cpu::ops::swiglu_mul(&mut cpu, &up);
    ...
    fn matvec_iq4_xs_matches_dequant_path() {
        let Some(cache) = skip_if_no_gpu() else { return };
        use reinstinct_quant::iq4_xs::{BLOCK_SIZE, BYTES_PER_BLOCK};
        use reinstinct_quant::half::f32_to_f16;
        ...
        reinstinct_quant::iq4_xs::dequantize_to_f32(&w_bytes, &mut w_fp32);
        let mut cpu = vec![0.0f32; out_dim];
        reinstinct_cpu::ops::matvec(&x, &w_fp32, in_dim, out_dim, &mut cpu);
    ...
    fn matvec_q5_k_matches_dequant_path() {
        ...
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q5_k::dequantize_to_f32(&w_bytes, &mut w_fp32);
        let mut cpu = vec![0.0f32; out_dim];
        reinstinct_cpu::ops::matvec(&x, &w_fp32, in_dim, out_dim, &mut cpu);
    ...
    fn matvec_q6_k_matches_dequant_path() {
        ...
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q6_k::dequantize_to_f32(&w_bytes, &mut w_fp32);
        let mut cpu = vec![0.0f32; out_dim];
        reinstinct_cpu::ops::matvec(&x, &w_fp32, in_dim, out_dim, &mut cpu);
    ...
    fn matvec_q4_k_dp4a_matches_dequant_path() {
        ...
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q4_k::dequantize_to_f32(&w_bytes, &mut w_fp32);
        let mut cpu = vec![0.0f32; out_dim];
        reinstinct_cpu::ops::matvec(&x, &w_fp32, in_dim, out_dim, &mut cpu);
    ...
    fn matvec_q4k_repacked_matches_dequant_path() {
        ...
        let packed = reinstinct_quant::q4_k::repack_for_matvec(&w_bytes, in_dim, out_dim);
        ...
        {
            use reinstinct_quant::q5_k::{BLOCK_SIZE as Q5_BS, BYTES_PER_BLOCK as Q5_BPB};
            ...
            reinstinct_quant::q5_k::dequantize_to_f32(&wb, &mut wf);
            let mut cpu5 = vec![0.0f32; out_dim];
            reinstinct_cpu::ops::matvec(&x, &wf, in_dim, out_dim, &mut cpu5);
            let p5 = reinstinct_quant::q5_k::repack_for_matvec(&wb, in_dim, out_dim);
        ...
        {
            use reinstinct_quant::q6_k::{BLOCK_SIZE as Q6_BS, BYTES_PER_BLOCK as Q6_BPB};
            ...
            let mut wf = vec![0.0f32; out_dim * in_dim];
            reinstinct_quant::q6_k::dequantize_to_f32(&wb, &mut wf);
            let mut cpu6 = vec![0.0f32; out_dim];
            reinstinct_cpu::ops::matvec(&x, &wf, in_dim, out_dim, &mut cpu6);
            let p6 = reinstinct_quant::q6_k::repack_for_matvec(&wb, in_dim, out_dim);
    ...
    fn mmq_gemm_q4k_repacked_matches_matvec() {
        ...
        for p in 0..p_rows {
            ...
            reinstinct_cpu::ops::matvec(&x[p*in_dim..(p+1)*in_dim], &w_fp32,
    ...
    fn mmq_gemm_q5k_repacked_matches_matvec() {
        ...
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q5_k::dequantize_to_f32(&w, &mut w_fp32);
        let mut cpu = vec![0.0f32; p_rows * out_dim];
        for p in 0..p_rows {
            let mut row = vec![0.0f32; out_dim];
            reinstinct_cpu::ops::matvec(&x[p*in_dim..(p+1)*in_dim], &w_fp32,
        ...
        let packed = reinstinct_quant::q5_k::repack_for_matvec(&w, in_dim, out_dim);
    ...
    fn mmq_gemm_q6k_repacked_matches_matvec() {
        ...
        let x = mmq_test_x(p_rows, in_dim);
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q6_k::dequantize_to_f32(&w, &mut w_fp32);
        let mut cpu = vec![0.0f32; p_rows * out_dim];
        for p in 0..p_rows {
            let mut row = vec![0.0f32; out_dim];
            reinstinct_cpu::ops::matvec(&x[p*in_dim..(p+1)*in_dim], &w_fp32,
        ...
        let packed = reinstinct_quant::q6_k::repack_for_matvec(&w, in_dim, out_dim);
    ...
    fn bench_q4k_repacked_vs_dp4a() {
      ...
      for &(in_dim, out_dim) in &[(5376usize, 21504usize),       // gemma ffn out
        ...
        let packed = reinstinct_quant::q4_k::repack_for_matvec(&w, in_dim, out_dim);
        ...
        let stream = reinstinct_hip::Stream::new().unwrap();
        ...
        let bench = |module: &Module, kname: &str, wptr: *mut c_void, rows_per_wg: u32, bytes: f64| -> (f64, f64) {
            ...
            let (a, b) = (reinstinct_hip::Event::new().unwrap(), reinstinct_hip::Event::new().unwrap());
            a.record(&stream).unwrap();
            const N: usize = 300;
            for _ in 0..N { launch(); }
            b.record(&stream).unwrap();
            stream.synchronize().unwrap();
            let ms = reinstinct_hip::Event::elapsed_time(&a, &b).unwrap() as f64 / N as f64;
    ...
    fn matvec_q8_0_dp4a_matches_dequant_path() {
        ...
        let mut w_fp32 = vec![0.0f32; out_dim * in_dim];
        reinstinct_quant::q8_0::dequantize_to_f32(&w_bytes, &mut w_fp32);
        let mut cpu = vec![0.0f32; out_dim];
        reinstinct_cpu::ops::matvec(&x, &w_fp32, in_dim, out_dim, &mut cpu);
    ...
    fn matvec_matches_cpu_across_shapes() {
        ...
        for &(in_dim, out_dim) in &[(2048usize, 2048usize), (2048, 6144), (2560, 384), (4096, 4096)] {
            ...
            let mut cpu = vec![0.0f32; out_dim];
            reinstinct_cpu::ops::matvec(&x, &w, in_dim, out_dim, &mut cpu);
    ...
    fn check_bulk_dequant(cache: &KernelCache, name: &str, source: &str, kernel: &str,
        ...
        unsafe { f.launch((n_blocks as u32, 1, 1), (block_threads, 1, 1), 0, None, &mut args)
            .expect("launch"); }
        reinstinct_hip::Device(0).synchronize().expect("sync");
        ...
        use reinstinct_quant::half::f16_to_f32;
        let mut max_abs = 0.0f32;
    ...
    fn bulk_dequant_matches_cpu() {
        ...
        {
            let n_blocks = 64;
            let mut w = vec![0u8; n_blocks * reinstinct_quant::q4_k::BYTES_PER_BLOCK];
            for b in w.iter_mut() { *b = rng_u8(); }
            // Tame the fp16 scales so dequant values stay in fp16 range.
            for blk in 0..n_blocks {
                let off = blk * reinstinct_quant::q4_k::BYTES_PER_BLOCK;
                w[off..off+2].copy_from_slice(
                    &reinstinct_quant::half::f32_to_f16(0.01).to_le_bytes());
                w[off+2..off+4].copy_from_slice(
                    &reinstinct_quant::half::f32_to_f16(0.005).to_le_bytes());
            }
            let mut cpu = vec![0.0f32; n_blocks * 256];
            reinstinct_quant::q4_k::dequantize_to_f32(&w, &mut cpu);
        ...
        {
            let n_blocks = 64;
            let bpb = reinstinct_quant::q6_k::BYTES_PER_BLOCK;
            ...
            let mut cpu = vec![0.0f32; n_blocks * 256];
            reinstinct_quant::q6_k::dequantize_to_f32(&w, &mut cpu);
        ...
        {
            let n_blocks = 64;
            let bpb = reinstinct_quant::q5_k::BYTES_PER_BLOCK;
            let mut w = vec![0u8; n_blocks * bpb];
            for b in w.iter_mut() { *b = rng_u8(); }
            for blk in 0..n_blocks {
                let off = blk * bpb;
            ...
            let mut cpu = vec![0.0f32; n_blocks * 256];
            reinstinct_quant::q5_k::dequantize_to_f32(&w, &mut cpu);
        ...
        {
            let n_blocks = 256;
            let bpb = reinstinct_quant::q8_0::BYTES_PER_BLOCK;
            let mut w = vec![0u8; n_blocks * bpb];
            for b in w.iter_mut() { *b = rng_u8(); }
            for blk in 0..n_blocks {
                let off = blk * bpb;
                w[off..off+2].copy_from_slice(
                    &reinstinct_quant::half::f32_to_f16(0.02).to_le_bytes());
            }
            let mut cpu = vec![0.0f32; n_blocks * 32];
            reinstinct_quant::q8_0::dequantize_to_f32(&w, &mut cpu);
        ...
        {
            let n_blocks = 64;
            let bpb = reinstinct_quant::iq4_xs::BYTES_PER_BLOCK;
            let mut w = vec![0u8; n_blocks * bpb];
            for b in w.iter_mut() { *b = rng_u8(); }
            for blk in 0..n_blocks {
                let off = blk * bpb;
            ...
            let mut cpu = vec![0.0f32; n_blocks * 256];
            reinstinct_quant::iq4_xs::dequantize_to_f32(&w, &mut cpu);
